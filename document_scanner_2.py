...
class DocumentScannerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Document Scanner (Auto AF, ROI capture)")
        ...
        # Set AF to Auto mode by default
    ...
    def wait_for_focus(self, timeout_ms=1200):
        start = time.time()
        while (time.time() - start) * 1000 < timeout_ms:
            try:
                meta = self.picam2.capture_metadata()
                state = self.map_af_state(meta.get("AfState"))
                if state == "Focused" or meta.get("AfState") == 2:
                    return True
                if state == "Failed" or meta.get("AfState") == 3:
                    return False
            except Exception:
                pass
            time.sleep(0.02)
    ...
    def capture_image(self):
        # Trigger single-shot autofocus before capture
        self.set_controls_safe({"AfMode": AF_MODE_AUTO, "AfTrigger": AF_TRIGGER_START})
        focused = self.wait_for_focus(timeout_ms=1200)
