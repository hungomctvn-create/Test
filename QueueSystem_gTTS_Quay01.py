...
class QueueCounterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Quầy 01 - Gọi số")
        self.root.geometry("360x220")

        self.title_label = tk.Label(root, text="Quầy 01", font=("Arial", 18, "bold"))
    ...
    def call_next(self):
        try:
            import requests
            resp = requests.post(f"{SERVER_URL}/api/next", json={"counter_id": "01"}, timeout=5)
            data = resp.json()
            ticket_str = data.get("ticket")
            if ticket_str:
                message = f"Quầy 01. Mời số {ticket_str}."
